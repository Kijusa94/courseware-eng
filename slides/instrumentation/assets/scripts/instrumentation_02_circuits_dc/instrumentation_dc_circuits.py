...
def dc_circuit_kirchhoff_example():
    with schemdraw.Drawing(show=False) as d:
        c1 = elm.SourceV().label(f"$V_s$")
        c2 = elm.Resistor().right().label(f"$R_1$")
        c3 = elm.Resistor().down().label(f"$R_2$")
        elm.Line().tox(c1["end"])
        elm.Line().right().at(c3["start"])
        c5 = elm.Resistor().down().label(f"$R_3$")
        elm.Line().tox(c3["end"])
    return d